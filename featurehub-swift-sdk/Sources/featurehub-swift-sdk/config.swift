...
public class EdgeFeatureHubConfig: FeatureHubConfig {
    ...
    public init(_ featurehubUrl: String, _ apiKey: [String]) {
        ...
        edgeProvider = { (repo, config) in UseBasedEdge(repo, config, 360) }
    }
    ...
    public func useRest(_ timoutInSeconds: Int) {
        edgeProvider = { (repo, config) in UseBasedEdge(repo, config, timoutInSeconds) }
    }
